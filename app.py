...
with zakl[3]:
    if 'mrna' not in st.session_state:
        ...
        if 'ranking' in st.session_state:
            ...
            f_dist = st.slider(
                'Minimum distance between candidates (nt)', 0, 50, 0, 5,
                help='Candidates offset by one or two nucleotides target the '
                     'same site. Ordering several of them buys the same '
                     'target repeatedly. A value of 20 nt keeps only '
                     'non-overlapping sites, retaining the highest-scoring '
                     'candidate from each.')
            ...
            if f_dist > 0:
                przed_dist = len(widoczne)
                # zachowujemy najwyzej oceniony wariant z kazdego miejsca
                zachowane = []
                for c in sorted(widoczne, key=lambda x: -x['wynik']):
                    if all(abs(c['poz_start_1based'] - z['poz_start_1based'])
                           >= f_dist or c['dlugosc'] != z['dlugosc']
                           for z in zachowane):
                        zachowane.append(c)
                widoczne = sorted(zachowane, key=lambda c: c['ranga'])
                st.caption(
                    f'Showing **{len(widoczne)}** of {len(wyn)} — '
                    f'{przed_dist - len(widoczne)} removed as overlapping '
                    f'within {f_dist} nt of a higher-scoring candidate.')
            else:
            ...
            if not widoczne:
                ...
                if widok == 'Nested series':
                    ...
                    try:
                        serie = design.serie_zagniezdzone(
                            widoczne, tolerancja_poz=tol, kotwica='koniec')
                    except TypeError:
                        st.error(
                            'The installed **design.py** is an older version '
                            'that anchors nested series on the window start '
                            'rather than its end. Anchoring on the start '
                            'gives variants with different seed regions, '
                            'which confounds length with seed identity and '
                            'makes the comparison uninterpretable. '
                            'Upload the current design.py.')
                        serie = []
                    if not serie:
                        st.warning(
                        ...
                        for i, seria in enumerate(serie, 1):
                            ...
                            wiersz = {
                                'series': i,
                                'anchor_end': seria.get('_poz_kotwicy', '?'),
                                'lengths': ', '.join(f'{d}' for d in dl_list),
                                'complete': len(dl_list) == len(wszystkie_dl),
                                'same_seed': seria.get('_seed_zgodny', False),
                ...
                # Arkusz zbiorczy jako pierwszy: plik otwarty w Excelu
                # pokazuje pierwszy arkusz, wiec bez tego uzytkownik widzi
                # wylacznie pierwsza grupe i moze uznac, ze reszty brakuje.
                if len(grupy) > 1:
                    wszystko = []
                    for nazwa_gr, wiersze_gr in grupy.items():
                        for w in wiersze_gr:
                            w = dict(w)
                            w['group'] = nazwa_gr
                            wszystko.append(w)
                    kol_xlsx = ['group'] + kol_eksport
                    grupy_xlsx = {'ALL': wszystko, **grupy}
                else:
                    kol_xlsx = kol_eksport
                    grupy_xlsx = grupy

                dane_xlsx = do_xlsx(grupy_xlsx, kol_xlsx)
                if dane_xlsx is not None:
                    e3.download_button(
                        ...
                        'spreadsheetml.sheet',
                        use_container_width=True,
                        help='Excel workbook. The first sheet, ALL, holds '
                             'every candidate; further sheets hold one group '
                             'each. Sheet tabs are at the bottom of the '
                             'Excel window.')
                else:
